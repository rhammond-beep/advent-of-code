...
func main() {
	...
	for i := range input {
		for j := range input {
			...
			if j < len(input)-2 && input[i][j+1] == 'M' && input[i][j+2] == 'A' && input[i][j+3] == 'S' {
				ans += 1
			}
			...
			if i < len(input)-2 && input[i+1][j] == 'M' && input[i+2][j] == 'A' && input[i+3][j] == 'S' {
				ans += 1
			}
			...
			// top right-diagonal
			if i > 2 && j < len(input)-2 && input[i-1][j+1] == 'M' && input[i-2][j+2] == 'A' && input[i-3][j+3] == 'S' {
				ans += 1
			}

			// bottom right-diagonal
			if i < len(input)-2 && j < len(input)-2 && input[i+1][j+1] == 'M' && input[i+2][j+2] == 'A' && input[i+3][j+3] == 'S' {
				ans += 1
			}

			// bottom left-diagonal
			if i < len(input)-2 && j > 2 && input[i+1][j-1] == 'M' && input[i+2][j-2] == 'A' && input[i+3][j-3] == 'S' {
				ans += 1
			}
